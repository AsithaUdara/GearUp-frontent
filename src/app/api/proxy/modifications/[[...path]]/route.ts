...
function getTargetBase(): { url: string; viaGateway: boolean; fallbackUrl: string } {
  ...
  const gw = `${getGatewayBase()}/api/modifications`;
  const direct = `${getDirectServiceBase()}/api/modifications`;
...
async function proxy(req: NextRequest, params: { path?: string[] }) {
  ...
  try {
    console.log(`[modifications-proxy] ${req.method} ${url}`);
    const resp = await fetch(url, init);
    console.log(`[modifications-proxy] response status: ${resp.status}`);
    ...
    // If gateway path failed and we intended to use gateway, try direct service as fallback
    if (viaGateway) {
      const alt = `${fallbackUrl}${suffix ? '/' + suffix : ''}${qs}`;
      try {
        console.warn(`[modifications-proxy] primary failed, retrying direct service: ${alt}`);
        ...
        console.error(`[modifications-proxy] fallback fetch error for ${alt}:`, e2);
      }
    }
    console.error(`[modifications-proxy] fetch error for ${url}:`, err);
