...
import { Appointment } from "../../admin/appointment/page";
import { NextRequest } from "next/server";

// Mock data for appointments (replace with actual database fetching)
...
export async function GET() {
  // Filter appointments to only include those with a requestedBay (assigned employee)
  const assignedAppointments = mockAppointments.filter(
    (appointment) => appointment.requestedBay !== undefined
  );
  return NextResponse.json(assignedAppointments);
}

export async function POST(request: NextRequest) {
  const newAppointment = await request.json();
  newAppointment.id = mockAppointments.length + 1;
